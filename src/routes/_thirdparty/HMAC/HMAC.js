...
export function HmacHex (key, data) {
  return hex(sign(key, data))
}

export function HmacRandom (key, data) {
  // Returns HMAC-generated random number between 0 and 1 (40-bit precision)
  const n = 10 // No. of hex digits to keep
  const x = parseInt('0x' + HmacHex(key, data).substring(0, n))
  return x / 2 ** (4 * n)
}
